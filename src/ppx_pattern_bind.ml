...
let name_expr expr = (* to avoid duplicating non-value expressions *)
...
type pat_exh = { pat : pattern; assume_is_exhaustive : bool }
...
let project_bound_var ~loc exp ~pat:{ pat; assume_is_exhaustive } var =
  let project_the_var =
    ...
    let pattern =
      replace_variable pat
        ~f:(fun v -> if String.equal v.txt var.txt then `Rename tmpvar else `Remove)
    in
  ...
  in
  Ppx_let_expander.expand ~modul:None Map
  ...
  let rhs =
    projected_vars_rhs extension ~loc
      ~bindings:(project_bound_vars ~loc exp
                   ~lhs:{ pat = match_case.pc_lhs
                        ; assume_is_exhaustive = assume_lhs_is_exhaustive })
...
let case_number_cases ~loc extension exp indexed_cases =
  ...
  List.map indexed_cases
    ~f:(expand_case extension exp ~assume_lhs_is_exhaustive)
  ...
  List.iter cases ~f:(fun { pc_lhs; pc_guard; _ } ->
    ...
    (match pc_guard with
     ...
     | Some v ->
       ...
       Location.raise_errorf ~loc:v.pexp_loc
...
let expand extension ~loc expr =
  ...
  | Pexp_let (rec_flag, vbs, exp) ->
    (match rec_flag with
     | Nonrecursive -> ()
     | Recursive ->
       Location.raise_errorf ~loc "%%pattern_%s cannot be used with 'let rec'"
         (string_of_extension extension));
    expand_let extension ~loc vbs exp
  | Pexp_match (expr, cases) -> expand_match extension ~loc expr cases
  | _ ->
    Location.raise_errorf ~loc "'%%pattern_%s can only be used with 'let' and 'match'"
      (string_of_extension extension)
;;
...
let () =
  Driver.register_transformation "pattern_bind"
